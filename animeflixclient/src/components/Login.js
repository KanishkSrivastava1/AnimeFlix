...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_Server}/api/auth/loginUser`, {
        method: 'POST',
...
{/*<img className="mb-4 m-auto d-flex justify-content-center" src={logo} alt="" width="72" height="57" />*/}
        <h1 className="h3 mb-3 my-5  fw-normal d-flex justify-content-center">Please Login</h1>

        <div className="form-floating my-2">
          <input className="form-control" onChange={handleOnChange} name="username" value={credentials.username} id="username" placeholder="Username" />
        ...
        <button className="btn btn-primary w-75 m-auto py-2 my-4 d-flex justify-content-center" type="submit">Login</button>
      </form>
      <div className='container '>
      <p className='m-2 d-flex justify-content-center'>You can use username as klaud</p> <p className='m-2 d-flex justify-content-center'>password as abcd </p>
      </div>
      </div>
      </main>
      </div>
